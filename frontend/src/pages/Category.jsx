...
const caratOf = (p) =>
    parseFloat(p.carat ?? p.carat_weight ?? p.diamond_weight ?? 0) || 0;
const purityTags = (p) =>
...
export default function CategoryPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            for (const path of [`/products/?category=${slug}`, `/products/?category_slug=${slug}`, "/products/"]) {
                try {
    ...
    const filtered = useMemo(() => {
        ...
        if (sel.price.length)
            list = list.filter((p) => sel.price.some((l) => PRICE_BUCKETS.find((b) => b.label === l)?.test(minPrice(p))));
        if (sel.carat.length)
            list = list.filter((p) => sel.carat.some((l) => CARAT_BUCKETS.find((b) => b.label === l)?.test(caratOf(p))));
    ...
    const FilterChip = ({ group, value }) => (
        <button
            ...
            className={`micro-label border px-3 py-2 transition-colors ${sel[group].includes(value)
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-10">
            ...
            <p className="micro-label text-charcoal/50 mb-4">
                <Link to="/" className="hover:text-gold">Home</Link> / Jewellery / {title}
            </p>
            <h1 className="text-4xl md:text-5xl font-serif mb-2">{title}</h1>
            <p className="text-sm text-charcoal/60 mb-10">{subtitle}</p>

            {/* Toolbar */}
            <div className="flex flex-wrap items-center justify-between gap-4 hairline border-b border-charcoal/15 pb-4 mb-8">
                <div className="flex items-center gap-6">
                    <button onClick={() => setShowFilters(!showFilters)} className="micro-label text-charcoal hover:text-gold">
                ...
                <label className="flex items-center gap-3 text-xs">
                    ...
                    <select
                        ...
                        className="border border-charcoal/25 bg-ivory px-3 py-2 text-xs focus:outline-none focus:border-gold"
                    >
            ...
            <div className={`grid gap-10 ${showFilters ? "lg:grid-cols-[260px_1fr]" : "grid-cols-1"}`}>
                ...
                {showFilters && (
                    <aside className="space-y-8">
                        ...
                        <div>
                            <h3 className="micro-label text-gold mb-3">Gold Purity &amp; Tone</h3>
                        ...
                        <div>
                            <h3 className="micro-label text-gold mb-3">Price Range (INR)</h3>
                            <div className="flex flex-wrap gap-2">
                        ...
                        <div>
                            <h3 className="micro-label text-gold mb-3">Diamond Weight</h3>
                            <div className="flex flex-wrap gap-2">
                        ...
                        <div>
                            <h3 className="micro-label text-gold mb-3">Diamond Quality</h3>
                            <div className="flex flex-wrap gap-2">
