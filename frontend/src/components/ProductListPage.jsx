...
export default function ProductListPage({ mode, slug, title, subtitle }) {
    ...
    // Fetch categories for shop mode filter
    useEffect(() => {
        if (isShopMode) {
            api.get("/categories/")
    ...
    const clearAll = () => {
        if (isShopMode) setSelectedCategory("");
        ...
        return (
            <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${active
                    ...
                    : "bg-white text-[#1A2536] border border-[#E5BDB0] hover:border-[#1A2536]"
                    }`}
            >
                {children || value}
            </button>
    ...
    const filterBody = (
        <div className="space-y-6">
            {/* Category Filter (shop mode only) */}
            {isShopMode && (
                <div>
                    <h3 className="text-xs uppercase tracking-[0.16em] font-bold text-[#1A2536] mb-3">Category</h3>
                    <div className="flex flex-wrap gap-2">
                        <button
                            onClick={() => setSelectedCategory("")}
                            className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${!selectedCategory
                                ? "bg-[#1A2536] text-white shadow-md"
                                : "bg-white text-[#1A2536] border border-[#E5BDB0] hover:border-[#1A2536]"
                                }`}
                        >
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${selectedCategory === cat.slug
                                    ? "bg-[#1A2536] text-white shadow-md"
                                    : "bg-white text-[#1A2536] border border-[#E5BDB0] hover:border-[#1A2536]"
                                    }`}
                            >
            ...
            {/* Subcategories (category mode only) */}
            {!isShopMode && subcategories.length > 0 && (
                <div>
            ...
            <div>
                ...
                <div className="flex justify-between items-center mt-2">
                    ...
                    <span className="text-xs font-extrabold text-[#1A2536]">
                        {sel.priceMax >= 200000
                            ? "Any Price"
            ...
            <div>
                ...
                <button
                    ...
                    className={`w-full flex items-center justify-between text-xs font-medium px-4 py-3 rounded-2xl border-2 transition-colors ${inStockOnly
                        ...
                        : "border-[#E5BDB0] bg-white text-[#1A2536] hover:border-[#1A2536]"
                        }`}
                >
                    In Stock Only
    ...
    while (p < items.length) {
        ...
        if (banner) {
            cells.push(
                <div key={`banner-${banner.position}`} className="relative overflow-hidden rounded-2xl shadow-xl border border-[#1A2536]/10 bg-[#1A2536] h-full">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#1A2536]/85 via-[#1A2536]/25 to-transparent" />
                    <div className="absolute inset-0 flex flex-col justify-end p-4 sm:p-6 text-white">
                        <span className="font-cursive text-xl sm:text-2xl text-[#E5BDB0] block -mb-1">crafted for you</span>
                        <p className="font-serif-luxury text-xl sm:text-2xl leading-tight mb-1">{banner.title}</p>
        ...
        <div className="bg-white pb-28 lg:pb-12">
            <div className="max-w-[1920px] mx-auto px-2 sm:px-4 lg:px-6 pt-8 pb-12 space-y-8">
                ...
                <p className="text-xs text-gray-500">
                    <Link href="/" className="hover:text-[#1A2536] underline-offset-4 hover:underline">Home</Link>
                    {isShopMode ? " / Shop All" : ` / Jewellery / ${title}`}
                </p>

                {/* Page Title (shop mode only) */}
                {isShopMode && (
                    <div className="space-y-2">
                        <span className="font-cursive text-3xl text-[#B86B5A] block -mb-1">explore everything</span>
                        <h1 className="font-serif-luxury text-4xl sm:text-5xl font-normal text-[#1A2536]">
                            All Fine Jewellery
                        </h1>
                        <p className="text-sm text-[#1A2536]/70 max-w-2xl">
                            Browse our complete collection of certified natural diamond jewellery. Filter by category, price, and more.
                        </p>
                    </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 glass-card-vibrant p-3 rounded-2xl border border-[#E5BDB0]">
                    <div className="flex items-center gap-3 text-xs">
                        ...
                        {activeCount > 0 && (
                            <button onClick={clearAll} className="text-[#1A2536] font-bold hover:underline underline-offset-4">
                                Clear Filters
                            </button>
                        )}
                    </div>
                    <label className="hidden lg:flex items-center gap-3 text-xs">
                        ...
                        <select
                            ...
                            className="border border-[#E5BDB0] bg-white px-4 py-2 text-sm rounded-full focus:outline-none focus:border-[#1A2536] font-medium"
                        >
                ...
                <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 glass-nav-vibrant px-4 py-3.5 flex items-center justify-around shadow-2xl">
                    ...
                    <button onClick={() => setShowMobileSort(true)} className="flex items-center gap-2.5 text-[#1A2536] font-bold text-sm">
                        Sort: <span className="text-[#1A2536]">{SORT_OPTIONS.find((o) => o.value === sort)?.label}</span>
                    </button>
                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-end justify-center lg:hidden">
                        ...
                        <div className="relative bg-white w-full max-h-[85vh] rounded-t-[32px] flex flex-col shadow-2xl border-t-2 border-[#E5BDB0]">
                            <div className="flex items-center justify-between p-5 border-b border-[#E5BDB0]/40">
                                ...
                                <button onClick={() => setShowMobileFilters(false)} className="text-[#1A2536]/60 hover:text-[#1A2536] text-2xl">✕</button>
                            </div>
                            ...
                            <div className="p-5 border-t border-[#E5BDB0]/40 flex items-center justify-between gap-4">
                                <button onClick={clearAll} className="text-xs uppercase tracking-[0.16em] font-bold text-[#1A2536] underline underline-offset-4">Clear All</button>
                        ...
                        <div className="relative bg-white w-full rounded-t-[32px] shadow-2xl p-2 border-t-2 border-[#E5BDB0]">
                            {SORT_OPTIONS.map((o) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-4 text-sm flex items-center justify-between rounded-2xl ${sort === o.value ? "text-[#1A2536] font-bold bg-white" : "text-[#1A2536]"
                                        }`}
                                >
                                    {o.label}
                                    {sort === o.value && <span className="text-[#1A2536]">✓</span>}
                                </button>
                            ))}
                        </div>
                    </div>
